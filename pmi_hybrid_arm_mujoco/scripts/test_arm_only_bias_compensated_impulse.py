#!/usr/bin/env python3
from __future__ import annotations

import csv
import sys
from pathlib import Path

_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

import mujoco as mj
import numpy as np

from kinematics.forward_kinematics import fk_ee_rp
from utils.mujoco_helpers import PKG_ROOT, joint_id, load_mjmodel

MODEL = PKG_ROOT / "models" / "pmi_arm_only_torque_debug.xml"
OUT = PKG_ROOT / "debug_outputs" / "arm_only_debug"
J = ["jnt1", "jnt2", "jnt3", "jnt4"]
RATIOS = np.array([0.5333, 0.15, 0.3, 0.3], dtype=float)
QJ0 = RATIOS * np.array([1.26513926, 8.49979867, 4.72038433, -1.50169410], dtype=float)


def reset_state(model, data):
    for i, n in enumerate(J):
        data.qpos[int(model.jnt_qposadr[joint_id(model, n)])] = QJ0[i]
    data.qvel[:] = 0.0
    mj.mj_forward(model, data)


def main() -> None:
    model = load_mjmodel(MODEL, strip_position_actuators=True)
    data = mj.MjData(model)
    scratch = mj.MjData(model)
    dof = np.array([int(model.jnt_dofadr[joint_id(model, n)]) for n in J], dtype=int)
    dt = float(model.opt.timestep)
    steps = max(1, int(round(0.05 / dt)))

    rows = []
    for ji, jn in enumerate(J):
        for delta in [1.0, 5.0, 10.0]:
            for sgn in [+1.0, -1.0]:
                reset_state(model, data)
                q0 = np.array([float(data.qpos[int(model.jnt_qposadr[joint_id(model, n)])]) for n in J])
                p0, *_ = fk_ee_rp(model, scratch, q0, J)
                p0 = np.asarray(p0)

                for _ in range(steps):
                    mj.mj_forward(model, data)
                    tau_bias = np.array([float(data.qfrc_bias[d]) for d in dof])
                    tau = tau_bias.copy()
                    tau[ji] += sgn * delta
                    data.qfrc_applied[:] = 0.0
                    for k in range(4):
                        data.qfrc_applied[dof[k]] = tau[k]
                    mj.mj_step(model, data)

                q1 = np.array([float(data.qpos[int(model.jnt_qposadr[joint_id(model, n)])]) for n in J])
                p1, *_ = fk_ee_rp(model, scratch, q1, J)
                p1 = np.asarray(p1)
                dq = q1 - q0
                rows.append({
                    "joint": jn,
                    "delta_torque": float(delta),
                    "torque_sign": float(sgn),
                    "delta_q_target": float(dq[ji]),
                    "delta_q": dq.tolist(),
                    "delta_ee": (p1 - p0).tolist(),
                    "sign_ok": bool(np.sign(dq[ji]) == np.sign(sgn) or abs(dq[ji]) < 1e-12),
                })

    OUT.mkdir(parents=True, exist_ok=True)
    with open(OUT / "bias_compensated_impulse.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)

    bad = [r for r in rows if not r["sign_ok"]]
    rep = [
        "# Bias Compensated Impulse Report",
        "",
        f"- mismatched sign cases: {len(bad)}",
        "- +delta should yield +delta_q_target and -delta should yield -delta_q_target.",
    ]
    (OUT / "bias_compensated_impulse_report.md").write_text("\n".join(rep) + "\n", encoding="utf-8")


if __name__ == "__main__":
    main()
